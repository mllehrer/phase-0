...
// Refactored Code
/* I couldn't figure out how to make it run by the deadline, but after it was due I had a friend who understands JavaScript better help me understand what I did wrong; this violates the solo challenge rules which is why I want to declare that in advance. This one runs, so I want to include it for posterity.

var bagTypes = ["duffel", "backpack", "belt bag"];

var tools = ["shovel", "GPS", "blaster", "oxygen mask"];
...
var r = confirm("You are awake. You are alive. From the rubble of your spacecraft, you hear the sound of an alien wind. Astronaut, are you ready?");
    if (r === false) {
        alert("You were not ready, but you are far from home. The atmosphere is not as oxygen-rich as Earth's, and you die slowly but painlessly of asphyxiation.");
       return;
    } else {
        var bag = prompt("You will need supplies. Choose a bag. You have a duffel that is heavy but can hold three items, a backpack that is lighter and can hold two, and a belt bag that will not weigh you down at all but can only hold one thing. Which will you choose? Input \"duffel\", \"backpack\", or \"belt bag\".");
      yourBag.bagType = bag;
      alert("You have chosen the " + bag + ".");
     }

  var firstTool = prompt("There are a few tools left in the spacecraft. You can take up to three if you chose the duffel, two if you chose the backpack, and one if you chose the belt bag. The tools are a shovel, a gps, a blaster gun, and an oxygen mask. To fill your bag, input 'shovel', 'gps', 'blaster', or 'oxygen mask'.");
yourBag.items = [];
yourBag.items.push(firstTool);

alert("The " + firstTool + " has been added to your bag.");

  if (yourBag.bagType === "duffel"){
    var secondTool = prompt("You may choose two more tools. What is your next choice? Remember the options are 'shovel', 'gps', 'blaster', or 'oxygen mask'.");
    yourBag.items.push(secondTool);
    alert("The " + secondTool + " has been added to your bag.");
    var thirdTool = prompt("You may choose one more tool. What will it be? Remember the options are 'shovel', 'gps', 'blaster', or 'oxygen mask'.");
    yourBag.items.push(thirdTool);
    alert("The " + thirdTool + " has been added to your bag.");
  } else if (yourBag.bagType === "backpack"){
      var secondTool = prompt("You may choose one more tool. What will it be? Remember the options are 'shovel', 'gps', 'blaster', or 'oxygen mask'.");
    yourBag.items.push(secondTool);
    alert("The " + secondTool + " has been added to your bag.");
    } else {
    alert("Wow, you must be very brave. I hope that was the right choice.");
  ...
  if (yourBag.items.indexOf("oxygen mask") > -1){
     alert("It's probably a good idea to put on your oxgen mask.");
  } else {
    alert("Oh dear, looks like you didn't bring an oxygen mask. You die of asphyxiation.");
    return;
    ...
    r = confirm("Breathing normally again, you take stock of your surroundings. The only way forward seems to be through a clearing, but the ground looks soft and boggy. Do you risk it?");
    if (r === false){
      alert("Staying in one place gets you eaten by an alien carnivore. You never saw it coming.");
     return;
    } else {
    ...
    if (yourBag.bagType === "duffel"){
      ...
      r = confirm("The ground is soft under your feet, and you flounder a bit but manage to cross into a wide meadow carpeted with bright flowers. A structure sits at the far end of the clearing. It looks as if it were built by something. Do you approach?");
      if (r === false){
      alert("Prudent, but a bad idea. You wander around in the forest until you die of starvation.");
      return;
    } else {
    ...
    if (yourBag.items.indexOf("blaster") > -1){
      r = confirm("You have your blaster. Do you want to shoot it?");
      if (r === true) {
        alert("What is wrong with you? Game over. Close your laptop and feel guilty for a little while. It only wanted to help.");
        return;
      } else {
